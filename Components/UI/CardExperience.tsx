...
const CardExperience = ({
  ...
  return (
    <article className={`  p-5 rounded-2xl  hover:shadow-sm hover:shadow-sky-400/50 transition-all duration-300 text-sm`}>
      ...
      <div className="flex flex-wrap gap-2">
        {keywords.map((keyword, index) => (
          <span
            key={index}
            className="px-3 py-1 bg-sky-500/20 text-sky-200 rounded-full text-xs border border-sky-400/30"
          >
            {keyword}
          </span>
        ))}
